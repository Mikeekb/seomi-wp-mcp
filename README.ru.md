...
8. Пропишет в `AGENTS.md`/`CLAUDE.md` готовые `scp`/`rsync`/`ssh` команды для деплоя темы/плагина на прод и карту MCP-серверов с разделением project-scope vs user-scope — чтобы AI-агент сразу понимал, что есть SSH-канал и как самостоятельно добавить новый MCP-сервер (`claude mcp add ... --scope project|user`), а не предлагал сначала PhpStorm/JetBrains UI.
9. **Опционально настроит интеграцию с Яндекс.Метрикой** — если ты согласишься, `init` спросит OAuth-токен + id счётчика, поставит бандл-сервер `seomi-metrika-mcp` (Python) в `.claude/mcp-servers/yandex-metrika/` (со своим `.venv`), зарегистрирует MCP-сервер `yandex-metrika` в `.mcp.json` и положит скилл `yandex-metrika`. Полностью опционально и независимо от WordPress — см. [Интеграция с Яндекс.Метрикой](#интеграция-с-яндексметрикой).
...
| `seomi-wp-mcp update`                | Подтягивает свежую версию mu-плагина, пересобирает managed-блок в `AGENTS.md`/`CLAUDE.md` и доустанавливает интеграцию с Яндекс.Метрикой в проекты, созданные старой версией |
| `seomi-wp-mcp doctor`                | Диагностика: env, mu-плагин, регистрация MCP-сервера, плагины-зависимости, здоровье Метрики (креды, Python 3.12+, venv, `yandex-metrika` в `.mcp.json`) |
| `seomi-wp-mcp doctor --fix`          | Авто-починка: ставит/активирует Abilities API + MCP Adapter; ставит WP-CLI на прод, если его там нет; завершает отложенную установку Метрики, если креды уже есть |
| `seomi-wp-mcp init --metrika` / `--no-metrika` | Принудительно настроить Метрику (без вопроса) / полностью пропустить — работает и для `update` |
...
- **Python 3.12+** — только если используешь интеграцию с Яндекс.Метрикой. Если его нет, скилл и креды всё равно ставятся, а сборка MCP-сервера откладывается с предупреждением (доделать позже через `seomi-wp-mcp doctor --fix`).
...
`init` может опционально подключить проект к **Яндекс.Метрике**, чтобы AI-агенты тянули аналитику, управляли целями и работали с сегментами аудитории через MCP. Это **полностью опционально и независимо от WordPress** — можно добавить и в обычный проект.

Поставляется как бандл Python MCP-сервер (`seomi-metrika-mcp`, модуль `seomi_metrika`, FastMCP + httpx + pydantic + structlog). При настройке сервер копируется в клиентский проект в `.claude/mcp-servers/yandex-metrika/`, собирается в собственный Python-virtualenv (`.venv`) и регистрируется в `.mcp.json` под именем сервера `yandex-metrika`. Скилл `yandex-metrika` тоже кладётся в `.claude/skills/`.

### Что умеет

- **Цели** — создание, редактирование и удаление целей Метрики (Management API).
- **Отчёты аналитики** — прогон отчётов и чтение счётчиков, целей и кампаний Яндекс.Директа.
- **Сохранение отчётов локально** — прогнать отчёт и сохранить его в `.ai-factory/metrika-reports/<date>_<slug>.json` + `.md`.
- **Сегменты аудитории** — список, создание, редактирование и удаление сегментов (Management API).

### Инструменты (13, все с префиксом `yandex_metrika_`)

| Группа | Инструменты |
|--------|-------------|
| Чтение / аналитика | `get_counter_info`, `list_counters`, `list_goals`, `get_report`, `list_direct_campaigns` |
| Цели (запись) | `create_goal`, `update_goal`, `delete_goal` |
| Сегменты (аудитория) | `list_segments`, `create_segment`, `update_segment`, `delete_segment` |
| Отчёты | `save_report` (прогоняет отчёт и сохраняет `.json` + `.md` в `.ai-factory/metrika-reports/`) |

### Credentials

Креды Метрики лежат в `.claude/.env` (gitignored) — **никогда** в `.mcp.json` (который коммитится):

| Ключ                  | Назначение                                                              |
|-----------------------|------------------------------------------------------------------------|
| `METRIKA_OAUTH_TOKEN` | OAuth-токен с https://oauth.yandex.ru/. Для отчётов достаточно `metrika:read`; для создания/редактирования целей и сегментов нужен `metrika:write`. |
| `METRIKA_COUNTER_ID`  | Id счётчика — берётся в интерфейсе Метрики (Настройки → номер счётчика). Несколько счётчиков — через запятую, например `43286099,46188792`. |

OAuth-токен читается только из `.claude/.env` и никогда не пишется в `.mcp.json`, так что коммит `.mcp.json` его не утечёт.

### Доустановка в существующий / старый проект

Если проект создан старой версией `seomi-wp-mcp` (когда Метрики ещё не было), доустановить её можно без полного ре-инита:

```
seomi-wp-mcp update            # предложит настройку Метрики интерактивно; неполную установку доделывает без вопросов
seomi-wp-mcp update --metrika  # принудительно настроить Метрику, без вопроса
seomi-wp-mcp doctor --fix      # доделать отложенную установку, если креды уже есть (сам креды не выдумывает)
```

Если Метрика уже настроена, `update` просто обновляет сервер.

### Требование Python 3.12+ и graceful degradation

Серверу нужен **Python 3.12+** на клиентской машине, но только если ты используешь Метрику. Если Python отсутствует на момент настройки — скилл и креды всё равно ставятся, а сборка venv откладывается с предупреждением: поставь Python 3.12+ позже и запусти `seomi-wp-mcp doctor --fix`, чтобы доделать. `doctor` показывает здоровье Метрики (креды на месте, доступен ли Python 3.12+, собран ли venv, есть ли `yandex-metrika` в `.mcp.json`).
...
| `METRIKA_OAUTH_TOKEN`     | OAuth-токен Яндекс.Метрики (с https://oauth.yandex.ru/) — scope `metrika:write` для целей/сегментов |
| `METRIKA_COUNTER_ID`      | Id счётчика(ов) Яндекс.Метрики, через запятую для нескольких (например `43286099,46188792`) |
...
skills/
  ...
  yandex-metrika/       Скилл Яндекс.Метрики (бандл, копируется в .claude/skills/)
mcp-servers/
  yandex-metrika/       Бандл Python MCP-сервер (seomi-metrika-mcp), ставится
                        в .claude/mcp-servers/ со своим .venv
templates/
